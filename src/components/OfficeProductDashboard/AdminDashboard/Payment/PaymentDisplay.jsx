...
import { Table, Monitor, Smartphone, Globe } from "lucide-react";
    ...
    queryFn: async () => {
      try {
        const res = await fetch(
          `${import.meta.env.VITE_COMMON_ROOT}/api/v1/payment/all_payment_list_admin`,
          {
            method: "GET",
            headers: {
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white border border-gray-200 shadow-sm rounded-lg">
          ...
          <tbody className="divide-y divide-gray-200">
            { !isLoading &&  allpaymentList?.success && allpaymentList?.data?.result?.sort((a,b)=>b?._id-a?._id).map((payment) => (
              <tr key={payment._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-sm text-gray-900">{payment.userId.os}</div>
                      <div className="text-sm text-gray-500">{payment.userId.browser}</div>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-4">
                ...
                {/* /fast_office_product/specific_office_categorie/:categorieId */}
                <td className="px-6 py-4">
                  <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                   <Link to={`/fast_office_product/specific_office_categorie/${payment?.categorieId}`} className='btn btn-outline btn-sm'>Categorie</Link>
                  </span>
                </td>
                <td className="px-6 py-4">
                  <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                   <Link to={`/fast_office_product/selling_product_details_parents/${payment?.categorieId}`} className='btn btn-outline btn-sm'>Office</Link>
                  </span>
                </td>
              </tr>
            ))}
